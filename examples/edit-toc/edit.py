...
class ScratchPad:
    def __init__(self):
        ...
        self.timezone = "%s'%s'" % (
            str(time.timezone / 3600).rjust(2, "0"),
...
def pdf_show(dlg, seite):
    ...
    spad.height = pix.h
    a = pix.samples  # samples without alpha bytes
    bmp = wx.Bitmap.FromBuffer(pix.w, pix.h, a)
    pix = None
    a = None
    return bmp
...
class MyGrid(gridlib.Grid):
    def __init__(self, parent):
        ...
        self.SetDefaultCellFont(
            wx.Font(
                wx.NORMAL_FONT.GetPointSize(), 70, 90, 90, False, "DejaVu Sans Mono"
            )
        )
    ...
    def OnIdle(self, evt):
        ...
        if self.bookmark > 0:  # did height changes happen?
            ...
            self.bookmark = -1
        if (time.time() - spad.lastsave) > 60.0 and spad.lastsave >= 0.0:
            dlg.auto_save()
        evt.Skip()
...
class PDFDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        explain = wx.StaticText(self, wx.ID_ANY, msg_txt, defPos, wx.Size(-1, 40), 0)
        ...
        tx_input = wx.StaticText(self, wx.ID_ANY, "Input:", defPos, defSiz, 0)
        self.metagrid.Add(tx_input, 0, wx.ALIGN_RIGHT, 5)

        tx_eindat = wx.StaticText(
            self,
            wx.ID_ANY,
        ...
        self.ausaut = wx.TextCtrl(
            self, wx.ID_ANY, spad.meta["author"], defPos, wx.Size(480, -1), wx.NO_BORDER
        )
        ...
        self.austit = wx.TextCtrl(
            self, wx.ID_ANY, spad.meta["title"], defPos, wx.Size(480, -1), wx.NO_BORDER
        )
        self.metagrid.Add(self.austit, 0, wx.LEFT, 5)

        tx_subject = wx.StaticText(self, wx.ID_ANY, "Subject:", defPos, defSiz, 0)
        ...
        self.btn_chk = wx.Button(
            self, wx.ID_ANY, "Check Data", defPos, defSiz, wx.BU_EXACTFIT
        )
        self.szr31.Add(self.btn_chk, 0, wx.ALIGN_TOP | wx.ALL, 5)
        self.msg = wx.StaticText(
            self,
            wx.ID_ANY,
        ...
        self.szr40OK = wx.Button(self, wx.ID_OK, "SAVE", defPos, defSiz, wx.BU_EXACTFIT)
        self.szr40OK.Disable()
        self.szr40.AddButton(self.szr40OK)
        self.szr40Cancel = wx.Button(
        ...
        max_pages = wx.StaticText(
            self, wx.ID_ANY, "of %s pages" % (str(spad.seiten),), defPos, defSiz, 0
        )
        ...
        bmp = pdf_show(self, 1)
        self.PDFbild = wx.StaticBitmap(
            self, wx.ID_ANY, bmp, defPos, defSiz, wx.BORDER_NONE
        )
    ...
    def auto_save(self):
        ...
        d = {
            "toc": self.tocgrid.Table.data,
            "author": self.ausaut.Value,
            "title": self.austit.Value,
            "subject": self.aussub.Value,
            "keywords": self.keywords.Value,
        }
        json.dump(d, f_toc)
        f_toc.close()
        spad.lastsave = time.time()
        return
    ...
    if os.path.exists(tocfile):
        ...
        if rc == wx.ID_YES:
            try:
                ...
                d = wx.MessageDialog(
                    None, "Ignoring saved data", "Invalid input from previous session"
                )
                d.ShowModal()
                d.Destroy()
                d = None
...
def make_pdf(dlg):
    ...
    if not spad.doc.can_save_incrementally():
        ...
        while outfile == spad.file:
            sdlg = wx.FileDialog(
                None, title, odir, "", "PDF files (*.pdf)|*.pdf", wx.FD_SAVE
            )
            if sdlg.ShowModal() == wx.ID_CANCEL:
                return None
            outfile = sdlg.GetPath()
...
if not infile:
    d = wx.FileDialog(
        None,
        message="Choose a PDF file to edit",
