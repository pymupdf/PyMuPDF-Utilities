...
* **doc-browser.py** - a complete GUI document displaying script using **Tkinter**! It it requires [PySimpleGUI](https://pypi.org/project/PySimpleGUI/), an awesome pure Python package exclusively based on Tkinter. To run the script, a PySimpleGUI version **greater 2.9.0** is required. For the time being, get it from the [latest](https://github.com/MikeTheWatchGuy/PySimpleGUI/tree/Dev-latest) development branch. It is a single Python file PySimpleGUI.py - you can use it without installation, put it where your import statement can find it.
